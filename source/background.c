...
int background_functions(
  ...
  if (pba->has_scf == _TRUE_) {
    ...
    /*
    if (pvecback_B[pba->index_bi_phi_scf] < 0){
       phi = fmod(pvecback_B[pba->index_bi_phi_scf], (-M_PI));
    }
    if (pvecback_B[pba->index_bi_phi_scf] > 0){
       phi = fmod(pvecback_B[pba->index_bi_phi_scf], M_PI);
    }
    */
    ...
    /*
    if ( ((a - 1./10.) < 0.0001 ) && (a > 1./10.)){
       phi_prime += pba->scf_parameters[2];
       pvecback_B[pba->index_bi_phi_prime_scf] = phi_prime;
    }
    */
    /*
    if ( (a > 1./10.)){
    ...
    */
    ...
    // pba->w_scf = ((phi_prime*phi_prime/(2*a*a) - V_scf(pba,phi))/3.)/((phi_prime*phi_prime/(2*a*a) + V_scf(pba,phi))/3.); 
...
int background_w_fld(
  ...
  case SCF:
    class_alloc(pvecback_,
              pba->bg_size*sizeof(double),
              pba->error_message);

    class_call(background_at_z(pba,1.0/a-1,long_info,inter_closeby,&last_index_back_,pvecback_),
             pba->error_message,
             pba->error_message);

    *w_fld =  pvecback_[pba->index_bg_p_scf]/pvecback_[pba->index_bg_rho_scf];
    break;
  }
...
int background_derivs(
  ...
  if (pba->has_scf == _TRUE_) {
    ...
    /*
    if ( ( a < (1./6. + 0.001) ) && (a > 1./6.)){
       y[pba->index_bi_phi_prime_scf] += pba->scf_parameters[2];
    ...
    */
    /*
    if ( y[pba->index_bi_phi_scf] > 0){
       y[pba->index_bi_phi_scf] = fmod(y[pba->index_bi_phi_scf],M_PI);
    }
    if ( y[pba->index_bi_phi_scf] < 0){
       y[pba->index_bi_phi_scf] = fmod(y[pba->index_bi_phi_scf],(-M_PI));
    }
    ...
    *//*
    ...
    //if ( ((a - 0.1) < 0.01 ) && (a > 0.1)){
        y[pba->index_bi_phi_prime_scf] += pba->scf_parameters[2];
        //y[pba->index_bi_phi_prime_scf] += (-3e-5);
    }*/
    dy[pba->index_bi_phi_scf] = y[pba->index_bi_phi_prime_scf]/(a*H);
    dy[pba->index_bi_phi_prime_scf] = - 2*y[pba->index_bi_phi_prime_scf] - a*dV_scf(pba,y[pba->index_bi_phi_scf])/H;
  }
...
double V_scf(struct background *pba, double phi) {
  if (POTENTIALTYPE == 0){
      ...
      return scf_m*scf_m*scf_f*scf_f*(1.-cos(phi/scf_f));
  }
  else if(POTENTIALTYPE == 1){
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return phi*pow(scf_mu,4)/scf_f;
  }
  else if(POTENTIALTYPE == 2){
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return pow(scf_mu,4)*exp(-phi/scf_f);
  }
  else if(POTENTIALTYPE == 3){
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return pow(scf_mu,4)*((M_PI/2.)+atan(-phi/scf_f));
  }
  else if(POTENTIALTYPE == 4){
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return scf_f*pow(scf_mu,4)/phi;
  }
  else if(POTENTIALTYPE == 5){
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return pow(scf_mu,4)*(exp(pow(phi/scf_f,2))-1);
  }
   else if(POTENTIALTYPE == 6){
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return pow(scf_mu,4)*exp(-pow(phi/scf_f,2));
  }
  else if(POTENTIALTYPE == 20){
      return 0.;
  }
  else if (POTENTIALTYPE == 7){
      double scf_m = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return scf_m*scf_m*scf_f*scf_f*pow(1.-cos(phi/scf_f),3);
  }
  else if (POTENTIALTYPE == 8){
      double scf_m = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return scf_m*scf_m*pow(phi,2)/2;
  }
}
...
double dV_scf(struct background *pba, double phi) {
  if (POTENTIALTYPE == 0){
      double scf_m = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return scf_m*scf_m*scf_f*sin(phi/scf_f);
  }
  else if(POTENTIALTYPE == 1){
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return pow(scf_mu,4)/scf_f;
  }
  else if(POTENTIALTYPE == 2){
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return -pow(scf_mu,4)*exp(-phi/scf_f)/scf_f;
  }
  else if(POTENTIALTYPE == 3){
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return -pow(scf_mu,4)/(1.+pow(-phi/scf_f,2));
  }
  else if(POTENTIALTYPE == 4){
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return -scf_f*pow(scf_mu,4)/pow(phi,2);
  }
  else if(POTENTIALTYPE == 5){
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return 2*pow(scf_mu,4)*exp(pow(phi/scf_f,2))*phi/pow(scf_f,2);
  }
  else if(POTENTIALTYPE == 6){
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      ...
      double scf_m = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return 3.0*scf_m*scf_m*scf_f*pow(1.-cos(phi/scf_f),2)*sin(phi/scf_f);
  }
  else if (POTENTIALTYPE == 8){
      double scf_m = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return scf_m*scf_m*phi;
  }
}
...
double ddV_scf(struct background *pba, double phi) {
    if (POTENTIALTYPE == 0){
        double scf_m = pba->scf_parameters[0];
        double scf_f = pba->scf_parameters[1];
        return scf_m*scf_m*cos(phi/scf_f);
    }
    else if(POTENTIALTYPE == 1){
        return 0.;
    }
      ...
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return pow(scf_mu,4)*exp(-phi/scf_f)/pow(scf_f,2);
  }
  else if(POTENTIALTYPE == 3){
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return 2*pow(scf_mu,4)*pow(scf_f,2)*phi/pow(pow(scf_f,2)+pow(phi,2),2);
  }
  else if(POTENTIALTYPE == 4){
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return 2.*scf_f*pow(scf_mu,4)/pow(phi,3);
  }
  else if(POTENTIALTYPE == 5){
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return (2*pow(scf_mu,4)*exp(pow(phi/scf_f,2))/pow(scf_f,2))*(1.+2.*pow(phi/scf_f,2));
  }
  else if(POTENTIALTYPE == 6){
      double scf_mu = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      ...
      double scf_m = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
      return 3.0*scf_m*scf_m*(1.-cos(phi/scf_f))*(2.*pow(sin(phi/scf_f),2) + pow(1.-cos(phi/scf_f),2)*cos(phi/scf_f) );
  }
  else if (POTENTIALTYPE == 8){
      double scf_m = pba->scf_parameters[0];
      double scf_f = pba->scf_parameters[1];
